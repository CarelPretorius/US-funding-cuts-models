...
for ii = 1:size(xs,1)
    ...
    incd(:,ii) = aux.incd; %diff(aux.soln(:,i.aux.inc(1)),[],1)*1e5; 
    mdr(:,ii)  = aux.mdr; %diff(aux.soln(:,i.aux.inc(2)),[],1)*1e5; 

end
fprintf('\n');

sim_pct = prctile(sims',[2.5,50,97.5],2);
incd_pct = prctile(incd,[2.5,50,97.5],2);  %1997-2019
mdr_pct = prctile(mdr,[2.5,50,97.5],2);  %1997-2019
