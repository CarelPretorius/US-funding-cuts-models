...
ix0 = 2e4; nx = 150; dx = round((size(xsto,1)-ix0)/nx); %2e4
xs = xsto(ix0:dx:end,:,1);
pct_xs= prctile(xs,[2.5,50,97.5]);

cs_red = [0.15, 0.15, 0.15];         % Percentage reduction in care-seeking (3 Scenarios)
disruption_dur = [3/12, 3/12, 3/12]; % Disruption period (3 months) 
recovery_dur = [20, 1, 3/12];        % Recovery period (year)from disruption[long, 0ne year, 3 months] 
...
for ii = 1:size(xs,1)
   ...
   % Find the initial condition at the starting point for the disruption
    geq = @(t,in) goveqs_basis2(t, in, M0, i, s, p, sel, agg); 
   ...
   dallmat = diff(allmat,[],1);
    ...
    inc_incr1 = (sum(inct(1:8,4,:)) - sum(inct(1:8,1,:)))./sum(inct(1:8,1,:)); % 2025 - 2030
    inc_incr2 = (sum(inct(1:8,3,:)) - sum(inct(1:8,1,:)))./sum(inct(1:8,1,:)); % 2025 - 2030
    inc_incr3 = (sum(inct(1:8,2,:)) - sum(inct(1:8,1,:)))./sum(inct(1:8,1,:)); % 2025 - 2030
    
    mor_incr1 = (sum(mort(1:8,4,:)) - sum(mort(1:8,1,:)))./sum(mort(1:8,1,:)); % 2025 - 2030
    mor_incr2 = (sum(mort(1:8,3,:)) - sum(mort(1:8,1,:)))./sum(mort(1:8,1,:)); % 2025 - 2030
    mor_incr3 = (sum(mort(1:8,2,:)) - sum(mort(1:8,1,:)))./sum(mort(1:8,1,:)); % 2025 - 2030
